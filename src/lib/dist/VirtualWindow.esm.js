...
function useObserver(measure) {
  ...
  var _measure = useCallback(measureFirstItem, [measure].concat(_toConsumableArray(deps)));
...
function useMeasurement() {
  var measure = useCallback(measureItem, []);
  var observer = useObserver(measure, []);
  var currentTarget = useRef(null); // a ref is just a function that is called
  // by React when an element is mounted
  // we use this to create an attach method
  // that immediately observes the size
  // of the reference

  var attach = useCallback(function attach(target) {
    if (!target) return;
...
var MeasuredContext = /*#__PURE__*/createContext({
  sizes: {},
  measuredId: 1,
  total: 0,
  count: 0,
...
function Measured(_ref) {
  var children = _ref.children,
      style = _ref.style,
      id = _ref.id;
  var context = useContext(MeasuredContext);
...
function useDebouncedRefresh() {
  ...
  var changed = useCallback(debounce(function () {
...
function VirtualWindow(_ref) {
  ...
  var previousTop = useRef(0);
  var changed = useDebouncedRefresh();
  var lastRendered = useRef([]);
  ...
  var measureContext = useMemo(function () {
    return {
      sizes: {},
      changed: changed,
      total: 0,
  ...
  var _useMemo = useMemo(render, [top, delta, expectedSize, totalCount, list, measureContext, windowHeight, measureContext.count, item, overscan]),
